...
#ifndef GENERALIZED_OFF_LATTICE_MODEL_2D_HH
#define GENERALIZED_OFF_LATTICE_MODEL_2D_HH
...
ExtrapolatedGeneralizedOffLatticeModel2D<T,Descriptor>::ExtrapolatedGeneralizedOffLatticeModel2D (
...
template<typename T, template<typename U> class Descriptor>
ExtrapolatedGeneralizedOffLatticeModel2D<T,Descriptor>::ExtrapolatedGeneralizedOffLatticeModel2D (
        ExtrapolatedGeneralizedOffLatticeModel2D<T,Descriptor> const& rhs )
    : OffLatticeModel2D<T,Array<T,2> >(rhs)
{ }

template<typename T, template<typename U> class Descriptor>
ExtrapolatedGeneralizedOffLatticeModel2D<T,Descriptor>&
...
template<typename T, template<typename U> class Descriptor>
ExtrapolatedGeneralizedOffLatticeModel2D<T,Descriptor>* ExtrapolatedGeneralizedOffLatticeModel2D<T,Descriptor>::clone() const {
    return new ExtrapolatedGeneralizedOffLatticeModel2D(*this);
}

template<typename T, template<typename U> class Descriptor>
plint ExtrapolatedGeneralizedOffLatticeModel2D<T,Descriptor>::getNumNeighbors() const {
    return 2;
}

template<typename T, template<typename U> class Descriptor>
void ExtrapolatedGeneralizedOffLatticeModel2D<T,Descriptor>::prepareCell (
        Dot2D const& cellLocation,
        AtomicContainerBlock2D& container )
{
    Dot2D offset = container.getLocation();
    ExtrapolatedGeneralizedOffLatticeInfo2D* info =
        dynamic_cast<ExtrapolatedGeneralizedOffLatticeInfo2D*>(container.getData());
    PLB_ASSERT( info );
...
template<typename T, template<typename U> class Descriptor>
ContainerBlockData*
    ExtrapolatedGeneralizedOffLatticeModel2D<T,Descriptor>::generateOffLatticeInfo() const
{
    return new ExtrapolatedGeneralizedOffLatticeInfo2D;
}

template<typename T, template<typename U> class Descriptor>
Array<T,2> ExtrapolatedGeneralizedOffLatticeModel2D<T,Descriptor>::getLocalForce (
...
template<typename T, template<typename U> class Descriptor>
void ExtrapolatedGeneralizedOffLatticeModel2D<T,Descriptor>::boundaryCompletion (
...
template<typename T, template<typename U> class Descriptor>
void ExtrapolatedGeneralizedOffLatticeModel2D<T,Descriptor>::cellCompletion (
        BlockLattice2D<T,Descriptor>& lattice,
        Dot2D const& genNode,
...
template<typename T, template<typename U> class Descriptor>
void ExtrapolatedGeneralizedOffLatticeModel2D<T,Descriptor>::computeVelocity (
          ...
          Array<T,2> const& wall_u, OffBoundary::Type bdType, Array<T,2> const& wallNormal,
...
template<typename T, template<typename U> class Descriptor>
InterpolatedGeneralizedOffLatticeModel2D<T,Descriptor>::InterpolatedGeneralizedOffLatticeModel2D (
        BoundaryShape2D<T,Array<T,2> >* shape_, int flowType_ )
    : OffLatticeModel2D<T,Array<T,2> >(shape_, flowType_)
{ }
...
template<typename T, template<typename U> class Descriptor>
InterpolatedGeneralizedOffLatticeModel2D<T,Descriptor>::InterpolatedGeneralizedOffLatticeModel2D (
        InterpolatedGeneralizedOffLatticeModel2D<T,Descriptor> const& rhs )
    : OffLatticeModel2D<T,Array<T,2> >(rhs)
{ }

template<typename T, template<typename U> class Descriptor>
InterpolatedGeneralizedOffLatticeModel2D<T,Descriptor>&
...
template<typename T, template<typename U> class Descriptor>
InterpolatedGeneralizedOffLatticeModel2D<T,Descriptor>* InterpolatedGeneralizedOffLatticeModel2D<T,Descriptor>::clone() const {
    return new InterpolatedGeneralizedOffLatticeModel2D(*this);
}

template<typename T, template<typename U> class Descriptor>
plint InterpolatedGeneralizedOffLatticeModel2D<T,Descriptor>::getNumNeighbors() const {
    return 2;
}

template<typename T, template<typename U> class Descriptor>
void InterpolatedGeneralizedOffLatticeModel2D<T,Descriptor>::prepareCell (
        Dot2D const& cellLocation,
        AtomicContainerBlock2D& container )
{
    Dot2D offset = container.getLocation();
    InterpolatedGeneralizedOffLatticeInfo2D* info =
        dynamic_cast<InterpolatedGeneralizedOffLatticeInfo2D*>(container.getData());
    PLB_ASSERT( info );
    if (this->isFluid(cellLocation+offset)) {
...
//                         pcout << "iPop = " << iPop << std::endl;
                        Dot2D nextNeighbor_neighbor(nextNeighbor.x+Descriptor<T>::c[iPop][0],
...
template<typename T, template<typename U> class Descriptor>
ContainerBlockData*
    InterpolatedGeneralizedOffLatticeModel2D<T,Descriptor>::generateOffLatticeInfo() const
{
    return new InterpolatedGeneralizedOffLatticeInfo2D;
}

template<typename T, template<typename U> class Descriptor>
Array<T,2> InterpolatedGeneralizedOffLatticeModel2D<T,Descriptor>::getLocalForce (
                AtomicContainerBlock2D& container ) const
{
    InterpolatedGeneralizedOffLatticeInfo2D* info =
        dynamic_cast<InterpolatedGeneralizedOffLatticeInfo2D*>(container.getData());
...
template<typename T, template<typename U> class Descriptor>
void InterpolatedGeneralizedOffLatticeModel2D<T,Descriptor>::boundaryCompletion (
        AtomicBlock2D& nonTypeLattice,
        AtomicContainerBlock2D& container,
        std::vector<AtomicBlock2D const*> const& args )
{
...
template<typename T, template<typename U> class Descriptor>
void InterpolatedGeneralizedOffLatticeModel2D<T,Descriptor>::cellCompletion (
        BlockLattice2D<T,Descriptor>& lattice,
        Dot2D const& genNode,
...
template<typename T, template<typename U> class Descriptor>
void InterpolatedGeneralizedOffLatticeModel2D<T,Descriptor>::computeVelocity (
          BlockLattice2D<T,Descriptor> const& lattice, Dot2D const& genNode,
          Dot2D const& solidDirection, int depth, Array<T,2> const& wallNode, T wallDistance, T cellDistance,
          Array<T,2> const& wall_u, OffBoundary::Type bdType, Array<T,2> const& wallNormal,
          Array<T,Descriptor<T>::d>& u) const
{
    //bool neumann = bdType==OffBoundary::neumann;
