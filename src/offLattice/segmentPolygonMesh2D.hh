    ...
    plint iVertex, plint jVertex ) const
{
    ...
    for ( ; tit != adjacentSegmentIds.end(); ++tit )
    {
        ...
        plint kVertex = ( iVertex != id0 && jVertex != id0 ) ? id0 : id1;
    ...
    plint iVertex, plint jVertex ) const
{
    ...
    for ( ; tit != neighborSegmentIds.end(); ++tit )
    {
        ...
        plint id0 = getVertexId ( iSegment, 0 );
        plint id1 = getVertexId ( iSegment, 1 );
        if ( ( iVertex == id0 || iVertex == id1 ) && ( jVertex == id0 || jVertex == id1 ) )
...
//FIXME
    ...
    PLB_ASSERT ( it != neighborVertexIds.end() ); // Vertices do not belong to the same segment.

//TODO which vertex is the prevVertex ?
    Array<T, 2> v0 = getVertex ( id0 );
    Array<T, 2> v1 = getVertex ( id1 );

    Array<T, 2> e01 = v1 - v0;

    Array<T, 2> n; //TODO ini
//     crossProduct(e01, e02, n);
    if ( !isAreaWeighted )
        n /= norm ( n );
    ...
    Array<T, 2> const& p, plint iSegment, bool isAreaWeighted ) const
{
    ...
    T area = computeSegmentLength ( iSegment );
...
template<typename T>
T SegmentPolygonMesh2D<T>::computeSegmentLength ( plint iSegment ) const
{
    Array<T, 2> v0 = getVertex ( iSegment, 0 );
    Array<T, 2> v1 = getVertex ( iSegment, 1 );
...
template<typename T>
T SegmentPolygonMesh2D<T>::computeSegmentLength (
    plint iVertex, plint jVertex ) const
{
    PLB_ASSERT ( iVertex >= 0 && iVertex < numVertices &&
                 jVertex >= 0 && jVertex < numVertices &&
...
template<typename T>
T SegmentPolygonMesh2D<T>::computeVertexLength ( plint iVertex ) const
{
    std::vector<plint> neighborSegmentIds = getNeighborSegmentIds ( iVertex );
    T length = T();
    std::vector<plint>::iterator it = neighborSegmentIds.begin();
    ...
    {
        length += computeSegmentLength ( *it );
    }
...
template<typename T>
T SegmentPolygonMesh2D<T>::computeAngle ( plint iVertex ) const
{
    std::vector<plint> adjacentSegmentIds = getAdjacentSegmentIds ( iVertex );
    ...
    Array<T, 2> const& point1, Array<T, 2> const& point2, plint iSegment ) const
{
    ...
    Array<T, 2> tv0 = getVertex ( iSegment, 0 );
    tv0.to_cArray ( v0 );
    Array<T, 2> tv1 = getVertex ( iSegment, 1 );
    tv1.to_cArray ( v1 );
    ...
    SegmentPolygonMesh2D<T> const& mesh, Curve2D const& lid )
{
    ...
    for ( plint iBoundary=0; iBoundary<numVertices; ++iBoundary )
    {
        ...
        length += mesh.computeSegmentArea ( centerVertex, vertex1, vertex2 );
    }
    return length;
}
