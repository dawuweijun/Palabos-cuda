...
#ifndef PARTICLE_VTK_2D_HH
#define PARTICLE_VTK_2D_HH
...
{
    SparseBlockStructure2D blockStructure(particles.getBoundingBox());
    ...
    MultiParticleField2D<DenseParticleField2D<T,Descriptor> > multiSerialParticles (
                    ...
                    std::vector<T> const& scalarFactor, std::vector<T> const& vectorFactor )
{
    ...
    std::vector<std::vector<Array<T,2> > > vectorData(vectors.size());
    for (pluint iVector=0; iVector<vectors.size(); ++iVector) {
        vectorData[iVector].resize(mesh.getNumVertices());
    }
    std::vector<Array<T,2> > posVect(mesh.getNumVertices());
    ...
    plint numWallSegments=0;
    for (plint iSegment=0; iSegment<mesh.getNumSegments(); ++iSegment) {
        if ( tag<0 || boundary.getTag(iSegment)==tag )
        {
            ++numWallSegments;
        }
    }

    ofile << "CELLS " << numWallSegments
          << " " << 4*numWallSegments << "\n";

    for (plint iSegment=0; iSegment<mesh.getNumSegments(); ++iSegment) {
        plint i0 = mesh.getVertexId(iSegment, 0);
        plint i1 = mesh.getVertexId(iSegment, 1);
        plint i2 = mesh.getVertexId(iSegment, 2);
        if ( tag<0 || boundary.getTag(iSegment)==tag )
        {
            ofile << "3 " << i0 << " " << i1 << " " << i2 << "\n";
        }
    ...
    ofile << "CELL_TYPES " << numWallSegments << "\n";
    for (plint i=0; i<numWallSegments; ++i) {
        ofile << "5\n";
    }
    ofile << "\n";
                         ...
                         std::vector<T> const& scalarFactor, std::vector<T> const& vectorFactor )
{
    ...
    for (pluint iParticle=0; iParticle<particles.size(); ++iParticle) {
        Array<T,2> position = particles[iParticle]->getPosition();
        ...
        std::string const& fName )
{
    ...
    if (global::mpi().isMainProcessor()) {
        ...
        for (pluint iParticle=0; iParticle<found.size(); ++iParticle) {
            Array<T,2> pos(found[iParticle]->getPosition());
            ofile << pos[0] << "," << pos[1] << "," << pos[2] << "\n";
        }
    }
}
        ...
        pluint maxNumParticlesToWrite)
{
    if (global::mpi().isMainProcessor()) {
        ...
        std::vector<Particle2D<T,Descriptor>*> found;
        SmartBulk2D oneBlockBulk(multiSerialParticles.getMultiBlockManagement(), 0);
        ...
        if (numParticlesToWrite == numParticles) {
            ...
            for (pluint i = 0; i < numParticlesToWrite; i++) {
                Array<T,2> pos(found[iParticles[i]]->getPosition());
                fprintf(fp, "% .7e % .7e % .7e\n", (float) pos[0], (float) pos[1], (float) pos[2]);
            }
        }
        ...
        for (; vectorIt != additionalVectors.end(); ++vectorIt) {
            ...
            if (numParticlesToWrite == numParticles) {
                for (pluint iParticle = 0; iParticle < numParticles; iParticle++) {
                    Array<T,2> vectorValue;
                    found[iParticle]->getVector(vectorID, vectorValue);
                    Array<float,2> floatVectorValue(vectorValue);
                    fprintf( fp, "% .7e % .7e % .7e\n",
...
void writeSelectedParticleVtk (
        ...
        std::map<plint,std::string> const& additionalVectors )
{
    MultiParticleField2D<DenseParticleField2D<T,Descriptor> > filteredParticles((MultiBlock2D&)particles);
    std::vector<MultiBlock2D*> particleArgs;
    particleArgs.push_back(&particles);
    particleArgs.push_back(&filteredParticles);
    applyProcessingFunctional (
            new CopySelectParticles2D<T,Descriptor>(tags.clone()),
            domain, particleArgs );
    SparseBlockStructure2D blockStructure(particles.getBoundingBox());
    blockStructure.addBlock(particles.getBoundingBox(), 0);
    plint envelopeWidth=1;
