...
BubbleMatch2D::BubbleMatch2D ( MultiBlock2D& templ, bool matchEmpty_ )
    : bubbleContainer ( createContainerBlock ( templ, new BubbleCounterData2D ( maxNumBubbles ) ) ),
      bubbleAnalysisContainer ( createContainerBlock ( templ, new BubbleAnalysisData2D() ) ),
      bubbleRemapContainer ( createContainerBlock ( templ, new BubbleRemapData2D ( maxNumBubbles ) ) ),
...
pluint BubbleMatch2D::countAndTagBubbles()
{
    std::vector<MultiBlock2D*> args;
    args.push_back ( tagMatrix );
    args.push_back ( bubbleRemapContainer );
    applyProcessingFunctional ( new CollectBubbleTags2D(), bubbleRemapContainer->getBoundingBox(), args );
    plint numBubbles=globalBubbleIds();
    applyProcessingFunctional ( new ApplyTagRemap2D(), bubbleRemapContainer->getBoundingBox(), args );
    return numBubbles;
}
...
void BubbleMatch2D::computeBubbleData ( pluint numBubbles )
{
    ...
    {
        plint id = localIds[i];
        AtomicContainerBlock2D& atomicDataContainer = bubbleAnalysisContainer->getComponent ( id );
        BubbleAnalysisData2D* pData = dynamic_cast<BubbleAnalysisData2D*> ( atomicDataContainer.getData() );
        PLB_ASSERT ( pData );
...
plint BubbleMatch2D::globalBubbleIds()
{
...
void BubbleMatch2D::resetBubbleContainer()
{
    MultiBlockManagement2D const& management = bubbleContainer->getMultiBlockManagement();
    ...
    for ( ; it != domains.end(); ++it, ++pos )
    {
        ...
        {
            AtomicContainerBlock2D& atomicDataContainer = bubbleContainer->getComponent ( id );
            dynamic_cast<BubbleCounterData2D*> ( atomicDataContainer.getData() )->reset();
        }
    }
}

void BubbleMatch2D::bubbleBucketFill ( MultiScalarField2D<int>& flag )
{
    setToConstant ( *tagMatrix, tagMatrix->getBoundingBox(), ( plint )-1 );
    ...
    while ( numIter>0 )
    {
        CountBubbleIteration2D functional ( matchEmpty );
        std::vector<MultiBlock2D*> args;
        args.push_back ( tagMatrix );
        args.push_back ( &flag );
...
bool BubbleCounterData2D::convertCell (
    plint& tag0,
...
CountBubbleIteration2D::CountBubbleIteration2D ( bool matchEmpty_ )
...
void CountBubbleIteration2D::processGenericBlocks ( Box2D domain,std::vector<AtomicBlock2D*> atomicBlocks )
{
    ...
    BubbleCounterData2D* pData = dynamic_cast<BubbleCounterData2D*> ( dataBlock.getData() );
    PLB_ASSERT ( pData );
        ...
        for ( plint iY=domain.y0; iY<=domain.y1; ++iY )
        {
                    ...
                    currentFlag==twoPhaseFlag::interface )
            {
                bool isConflicting = data.convertCell (
                                         tagMatrix.get ( iX  ,iY ),
                                         tagMatrix.get ( iX-1,iY ),
                                         tagMatrix.get ( iX  ,iY-1 ),
...
void CollectBubbleTags2D::processGenericBlocks ( Box2D domain,std::vector<AtomicBlock2D*> atomicBlocks )
{
    ...
    AtomicContainerBlock2D* pDataBlock = dynamic_cast<AtomicContainerBlock2D*> ( atomicBlocks[1] );
    PLB_ASSERT ( pDataBlock );
    AtomicContainerBlock2D& dataBlock = *pDataBlock;
    BubbleRemapData2D* pData = dynamic_cast<BubbleRemapData2D*> ( dataBlock.getData() );
    PLB_ASSERT ( pData );
    BubbleRemapData2D& data = *pData;
...
void ApplyTagRemap2D::processGenericBlocks ( Box2D domain,std::vector<AtomicBlock2D*> atomicBlocks )
{
    PLB_ASSERT ( atomicBlocks.size() ==2 );
    ScalarField2D<plint>* pTagMatrix = dynamic_cast<ScalarField2D<plint>*> ( atomicBlocks[0] );
    PLB_ASSERT ( pTagMatrix );
