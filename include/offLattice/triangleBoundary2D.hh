...
template<typename T>
Array<T,3> TriangleBoundary2D<T>::computeContinuousNormal (
...
template<typename T>
template<typename DomainFunctional>
plint TriangleBoundary2D<T>::tagDomain(DomainFunctional functional, Array<T,3> normal, T angleTolerance, plint previousTag)
{
    // Make sure we're working with the closed mesh.
    PLB_PRECONDITION( topology.top()==1 );
    ++currentTagNum;
...
template<typename T, class SurfaceData>
Array<T,3> TriangleFlowShape2D<T,SurfaceData>::computeContinuousNormal (
